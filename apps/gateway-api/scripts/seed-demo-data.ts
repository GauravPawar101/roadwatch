...
let pool: {
    ...
    // FIX: pool.end() must be part of the declared type — it was omitted
    // before, which is how a missing pool.end() call went unnoticed.
    end: () => Promise<void>;
};
...
// MITIGATION for root cause #2 (transaction/client leak in bulkUpsertRoads):
// The real fix belongs in ../src/db.js (see notes below), but that file
// wasn't provided here. As a caller-side safety net, wrap the call in a
// timeout so a leaked "IDLE in transaction" client causes a loud, fast
// failure instead of an indefinite hang.
function withTimeout<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {
    let timer: ReturnType<typeof setTimeout>;
    const timeout = new Promise<never>((_, reject) => {
        timer = setTimeout(() => {
            reject(new Error(`[seed-demo] ${label} timed out after ${ms}ms — likely a leaked pg client/transaction`));
        }, ms);
    });
...
async function main() {
    ...
    // REGRESSION FIX: initDb() must run before any table access — it applies
    // runtime setup (or at least verifies schema) that other services rely on.
    // This was accidentally dropped in an earlier restructuring; restoring it.
    await initDb();

    // CRITICAL FIX (root cause #1): pg.Pool keeps its sockets open and will
    // block the Node.js event loop from exiting on its own. Without an
    // explicit pool.end(), the script "finishes" (prints the summary table)
    // but the process never terminates. Wrapping the rest of main() in
    // try/finally guarantees pool.end() runs even if seeding throws partway
    // through, so a failed run doesn't leave the pool dangling either.
    try {
        await runSeedBody();
    } finally {
        await pool.end().catch((err: unknown) => {
            console.error('[seed-demo] pool.end() failed:', err);
        });
    }
      ...
      ON CONFLICT (country_code, state_code, code) DO UPDATE SET
        ...
        tile_style_url = EXCLUDED.tile_style_url`,
            ...
            await withTimeout(
                ...
                15_000,
                `bulkUpsertRoads(${district.districtCode})`
            );
        }
