...
docker-compose up -d kafka redis zookeeper --no-recreate 2>&1 | Out-Null
...
$fabricCmd = 'wsl -d Ubuntu -- bash -lc "source ~/.profile >/dev/null 2>&1; set -e; cd /mnt/c/Users/' + $env:USERNAME + '/Desktop/roadwatch/fabric/network; ./scripts/start.sh"'
