...
export class ImageSubmissionDB {
  ...
  async getSubmission(requestId: string): Promise<ImageSubmission | null> {
    const result = await pool.query<ImageSubmission>('SELECT * FROM image_submissions WHERE request_id = $1 LIMIT 1', [requestId]);
  ...
  async getUserKarmaTotal(userId: string): Promise<number> {
    const result = await pool.query<{ score: number }>('SELECT score FROM karma_records WHERE user_id = $1', [userId]);
    const row = result.rows[0];
  ...
  async getAuditsForSubmission(submissionId: string): Promise<VerificationAudit[]> {
    const result = await pool.query<VerificationAudit>('SELECT * FROM verification_audits WHERE submission_id = $1 ORDER BY created_at DESC', [submissionId]);
    return result.rows;
  }
  ...
  async getOrCreatePrivacyProfile(userId: string): Promise<UserPrivacyProfile> {
    const existingResult = await pool.query<UserPrivacyProfile>('SELECT * FROM user_privacy_profiles WHERE user_id = $1 LIMIT 1', [userId]);
    ...
    try {
      ...
      await client.query(
        ...
        INSERT INTO access_logs (
          ...
          accessed_fields, ip_address, user_agent, status, reason_blocked, created_at
        ) VALUES ($1, $2, $3, $4, $5, COALESCE($6::jsonb, '[]'::jsonb), $7, $8, $9, $10, NOW())
        `,
        [
          id,
          log.user_id,
          ...
          log.resource_id,
          log.action,
          log.accessed_fields ? JSON.stringify(log.accessed_fields) : null,
          log.ip_address ?? null,
          log.user_agent ?? null,
          log.status ?? null,
      ...
      const result = await client.query<AccessLog>('SELECT * FROM access_logs WHERE id = $1 LIMIT 1', [id]);
      await client.query('COMMIT');
      return result.rows[0]!;
    } catch (error) {
      await client.query('ROLLBACK');
